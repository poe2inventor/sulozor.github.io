...
const x0 = {
  ...
  , Fe = {
    ...
    crimson_hall: {
        ...
        variants: {
            tier1: {
                effects: ["15% chance for Rare Monsters to have +1 Modifier"]
            },
            tier2: {
                effects: ["30% chance for Rare Monsters to have +1 Modifier"]
            },
            tier3: {
                effects: ["60% chance for Rare Monsters to have +1 Modifier", "Adds Royal Sentinel", "Architect's Orb"]
            }
        },
        upgrades: {
            tier2: {
...
function l2() {
    return {
        contributingRooms: [],
        nextTierNeeds: null,
        specialInfo: "No adjacency upgrades. Gains +1 tier per Generator (stacks up to 2)."
    }
}
function i2(l, a, r) {
    const c = []
...
function __(l) {
    ...
    a[38] !== G || a[39] !== Q ? (oe = d.jsx("button", {
        className: G,
        onClick: Q,
        title: "Замок: Защищает комнату от дестабилизации",
        children: ae
    }),
    a[38] = G,
    a[39] = Q,
...
const as = [{
    version: "2.2.2",
    date: "Feb 13, 2026",
    title: "Исправление эффекта редкости сундука",
    changes: ["Исправлена ​​ошибка, из-за которой кузница (и другие комнаты с улучшенными характеристиками) не оказывали никакого эффекта без генератора", "Теперь в Кузнице корректно отображаются бонусы редкости сундуков даже без использования Генератора (Генератор по-прежнему дает +1 уровень)", "Это также исправляет проблему с отображением эффектов на первом уровне комнат Мастерская Големов, Плотосинтез и Бараки вознесенных, без питания"]
}, {
    version: "2.2.1c",
    date: "Feb 13, 2026",
...
const Gx = [{
    ...
    layout: "ACFIIUghSCFIIUhgOWA5YDlgSCE5ESkRKRE5IUhgKQCYACkySCE5EjEAeRFpIUhgcGl6cClwSCE5eXpwMRJ6IUgyaXBhOXGASCIxOWEIQGkxIg"
}, {
    id: "milkybk-hard",
...
function CS(l) {
    ...
    let z;
    a[5] !== r || a[6] !== m ? (z = ue => {
        confirm(`Overwrite "${ue}" with current layout?`) && (_r(ue, r),
        br(ue),
        X(),
        m(r, ue))
    }
...
function TS(l) {
    ...
    return u < 1 ? "just now" : u < 60 ? `${u}m ago` : m < 24 ? `${m}h ago` : f < 7 ? `${f}d ago` : a.toLocaleDateString()
}
function NS() {
    return Ly()
}
        ...
        children: [W, p ? d.jsxs("div", {
            ...
            children: [d.jsx("div", {
                ...
                children: [d.jsx("span", {
                    ...
                    children: p.targetedRoom ? p.isProtectedAttempt ? "🛡️ Protected" : "❌ Removed" : "Step"
                }), d.jsxs("span", {
                ...
                onClick: b,
                children: p.currentAttempt >= p.totalAttempts ? "View Results" : "Next Step →"
            })]
        }) : g ? d.jsxs("div", {
...
function i4() {
    ...
    l[5] !== J ? (vt = async () => {
        const be = await J();
        gt(be ? "Link copied to clipboard!" : "Failed to copy link")
    }
    ,
    l[5] = J,
    l[6] = vt) : vt = l[6];
    ...
    l[7] !== oe ? (fa = () => {
        const be = oe();
        be > 0 ? gt(`Applied medallion to ${be} room${be === 1 ? "" : "s"}`) : gt("No rooms available for medallion upgrade")
    }
    ,
    l[7] = oe,
    l[8] = fa) : fa = l[8];
    ...
    let Ta;
    l[47] !== w || l[48] !== St ? (Ta = () => {
        St && (gt(`Destabilized ${St.removedRooms.length} room(s)`),
        w(St.newGrid),
        Bt(null),
        tn(null),
        pl(null))
    ...
    l[53] !== w || l[54] !== T || l[55] !== E ? (Qn = be => {
        try {
            const Ae = Hu(be);
            Ae ? (w(Ae),
            gt("Preset loaded successfully!"),
            T(null),
            De(null),
            _t(),
            E(null)) : gt("Failed to load preset")
        } catch (Ae) {
            console.error("Error loading preset:", Ae),
            gt("Failed to load preset")
        }
    }
    ,
    l[53] = w,
    ...
    const _l = `${pe.gameModeBtn} ${ue ? pe.gameActive : ""}`
      , mo = ue ? "Switch to Planning Mode" : "Switch to Destabilization";
    let Yt;
...
//# sourceMappingURL=index-RwhIvrex.js.map
